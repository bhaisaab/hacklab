...
class SearchMasterServer(port: Int, id: Int) extends AbstractSearchServer(port, id) {
  ...
  override def query(q: String) = {
    ...
    //responses(0)
    val responsesF = Future.collect(clients.map {client =>
        client.query(q).map(resp => resp.getContent().toString(UTF_8))
    })

    responsesF.map { response => {
        queryMasterSuccessResponse(response.filter(_.size > 0).mkString(",\n"))
    }}
  }
}
